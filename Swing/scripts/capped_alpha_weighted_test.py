...
def compute_capped_alpha_weights(selected, max_weight=0.40):
    ...
    # cap weights and redistribute remaining weight
...
def simulate_capped_alpha_weighted(
    df,
    top_n=3,
    rebalance_every=3,
    ...
    for i in range(len(dates)):
        ...
        if should_rebalance:
            ...
            selected = compute_capped_alpha_weights(selected, max_weight=max_weight)
...
for cap in [0.40, 0.50, 0.60]:
    print("\n" + "=" * 60)
    print(f"CAPPED ALPHA WEIGHTED TEST | MAX WEIGHT {cap:.0%}")
    print("=" * 60)

    equity_curve, daily_returns = simulate_capped_alpha_weighted(
        df,
        top_n=3,
        rebalance_every=3,
        cost_per_rebalance=0.0005,
