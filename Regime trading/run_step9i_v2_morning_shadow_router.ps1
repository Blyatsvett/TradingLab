...
$Arguments = @("-m", "RegimeTrading.scripts.step9i_v2_core5_plus_holdout18_shadow_router", "morning")
if ($Date) { $Arguments += @("--date", $Date) }
if ($AsOf) { $Arguments += @("--as-of", $AsOf) }
if ($AllowLateReconstruction) { $Arguments += "--allow-late-reconstruction" }
& $Python @Arguments
if ($LASTEXITCODE -ne 0) { throw "Step 9I V2 morning router failed with exit code $LASTEXITCODE." }
