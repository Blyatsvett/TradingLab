...
def _step9l(args: argparse.Namespace) -> dict[str, Any]:
    ...
    batches, decisions, inserted = mod.seal_morning_decisions(
        ...
        export_outputs_after=False,
        simulated_clock=bool(args.as_of),
        include_core_regime_sensitivity=not bool(args.defer_core_regime_sensitivity),
    )
    batch = batches.iloc[0]
    return {
        "stage": "step9l",
        ...
        "eligible_rows": int(decisions["contract_eligible"].astype(bool).sum()),
        "active_guardrails": int(decisions["decision_action"].eq("GUARDRAIL_ACTIVE_AVOID_STRATEGY").sum()),
        "core_regime_sensitivity_deferred": bool(args.defer_core_regime_sensitivity),
        "router_active": False,
        "orders_enabled": False,
    }


def _step9l_sensitivity(args: argparse.Namespace) -> dict[str, Any]:
    from RegimeTrading.scripts import step9l_v3_selected_strategy_shadow_engine as mod
    ...
    prices = mod.load_shadow_prices(args.source_db)
    if prices.empty:
        raise mod.step9i.ShadowDataNotReady(
            f"No source prices found at {args.source_db}"
        )
    sensitivity = mod.complete_core_regime_sensitivity(
        target_date=target,
        prices=prices,
        ledger_db=args.ledger_db,
    )
    return {
        "stage": "step9l_sensitivity",
        "session_date": target,
        "sensitivity_rows": int(len(sensitivity)),
        "completion_status": "IMMUTABLE_DIAGNOSTIC_COMPLETION",
        "router_active": False,
        "orders_enabled": False,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    for name in ["step9i", "step9l"]:
        ...
        if name == "step9l":
            child.add_argument(
                "--defer-core-regime-sensitivity",
                action="store_true",
            )

    child = sub.add_parser("step9l-sensitivity")
    common(child)
    child.add_argument("--source-db", type=Path, required=True)
    child.add_argument("--ledger-db", type=Path, required=True)
...
def main() -> None:
    ...
    handlers = {
        "step9i": _step9i,
        "step9l": _step9l,
        "step9l-sensitivity": _step9l_sensitivity,
        "step9s": _step9s,
        "step9r": _step9r,
        "step9t": _step9t,
        "step9u": _step9u,
    ...
    if args.stage in {"step9i", "step9l", "step9s", "step9r", "step9t", "step9u"}:
        payload.setdefault("status", "STEP9_MORNING_V2_STAGE_PASSED")
