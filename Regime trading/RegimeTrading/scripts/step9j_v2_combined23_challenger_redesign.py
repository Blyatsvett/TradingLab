...
EXPERIMENT_ID = "STEP9J_V2_CHALLENGER_REDESIGN_COMBINED23_V1"
RESEARCH_STATUS = "SIMULATION_ONLY_POST_HOC_REDESIGN_DISCOVERY_NOT_CONFIRMATORY"
CODE_VERSION = "STEP9J_V2_COMBINED23_REDESIGN_CORE_EXECUTION_FIX_2026_07_26"
...
REGISTRY_FILE = legacy_output_path("step9j_v2_challenger_registry.csv")
SESSION_FILE = legacy_output_path("step9j_v2_session_coverage.csv")
CANDIDATE_FILE = legacy_output_path("step9j_v2_challenger_candidates.csv")
TRADE_FILE = legacy_output_path("step9j_v2_challenger_trades.csv")
LEG_FILE = legacy_output_path("step9j_v2_challenger_trade_legs.csv")
PERFORMANCE_FILE = legacy_output_path("step9j_v2_challenger_performance.csv")
COMPARISON_FILE = legacy_output_path("step9j_v2_challenger_comparisons.csv")
ROBUSTNESS_FILE = legacy_output_path("step9j_v2_challenger_robustness.csv")
MULTIPLE_TESTING_FILE = legacy_output_path("step9j_v2_challenger_multiple_testing.csv")
AUDIT_FILE = legacy_output_path("step9j_v2_challenger_audit.csv")
DIAGNOSTIC_FILE = legacy_output_path("step9j_v2_trade_diagnostics.csv")
TIME_SPLIT_FILE = legacy_output_path("step9j_v2_time_split_performance.csv")
TICKER_FILE = legacy_output_path("step9j_v2_ticker_performance.csv")
SECTOR_FILE = legacy_output_path("step9j_v2_sector_performance.csv")
SUMMARY_FILE = legacy_output_path("step9j_v2_summary.csv")
TAXONOMY_FILE = legacy_output_path("step9j_v2_daily_taxonomy.csv")
SEGMENT_PERFORMANCE_FILE = legacy_output_path("step9j_v2_segment_performance.csv")
...
CONTRACTS = [
    ...
    },
    {
        ...
        "cohort_id": "J_TU_ALL_COMPLETE_COMBINED23",
...
def build_segment_performance(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for segment, tickers in (("CORE_5", step9i.CORE_TICKERS), ("HOLDOUT_18", step9i.HOLDOUT_ONLY_TICKERS), ("COMBINED_23", step9i.TRADING_TICKERS)):
        ...
        performance = build_group_performance(subset, ["contract_id", "test_role", "primary_regime"])
...
def run_step9j(
    ...
    # Patch the shared Step 9B/Step 9H execution universe to all 23 tradable
    # tickers while the challenger engine is active.
    with step9i._patched_holdout_tickers():
    ...
    for frame in (trades, locked_trades):
        if not frame.empty and "universe_segment" not in frame.columns:
            frame.insert(frame.columns.get_loc("ticker") + 1, "universe_segment", frame["ticker"].map(step9i._segment_for_ticker))
...
def main() -> None:
    ...
    print("\n=== STEP 9J V2 CHALLENGER REDESIGN — COMBINED 23 ===")
    ...
    print("\nUse step9j_v2_challenger_performance.csv, step9j_v2_segment_performance.csv, step9j_v2_challenger_comparisons.csv, and step9j_v2_trade_diagnostics.csv for review.")
