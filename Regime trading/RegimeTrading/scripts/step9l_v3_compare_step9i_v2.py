...
DAILY_FILE = shadow_output_path("step9l_v3_vs_step9i_v2_daily_comparison.csv")
SUMMARY_FILE = shadow_output_path("step9l_v3_vs_step9i_v2_comparison_summary.csv")
AUDIT_FILE = shadow_output_path("step9l_v3_vs_step9i_v2_comparison_audit.csv")
...
def build_comparison(
    ...
    research_ledger: Path = step9l_v3.SHADOW_LEDGER_DB,
    ...
    research = _engine_daily("STEP9L_V3_RESEARCH", r_batches, r_decisions, r_outcomes)
    ...
    summary = pd.DataFrame([
        {
            "comparison": "STEP9L_V3_VS_STEP9I_V2_PRIMARY_HYPOTHESES",
    ...
    audit = pd.DataFrame([
        {
            "audit_item": "FROZEN_AND_V3_RESEARCH_LEDGERS_ARE_DISTINCT",
            "failures": 0 if distinct else 1,
            "audit_pass": distinct,
            "interpretation": "Step 9L V3 never writes to the frozen Step 9I V2 ledger.",
        },
        {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare frozen Step 9I V2 with Step 9L V3.")
    parser.add_argument("--frozen-ledger", type=Path, default=step9i.SHADOW_LEDGER_DB)
    parser.add_argument("--research-ledger", type=Path, default=step9l_v3.SHADOW_LEDGER_DB)
