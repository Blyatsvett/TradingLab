...
EXPERIMENT_ID = "STEP9IR_V2_HISTORICAL_WALK_FORWARD_REPLAY_23_TICKERS"
RESEARCH_STATUS = "SIMULATION_ONLY_HISTORICAL_REPLAY_NOT_CONFIRMATORY"
CODE_VERSION = "STEP9IR_V2_WALK_FORWARD_REPLAY_23_TICKERS_CORE_EOD_FIX_2026_07_26"
DEFAULT_START_DATE = "2026-05-25"
DEFAULT_END_DATE = "2026-07-24"

OUTPUT_DIR = LEGACY_OUTPUT_DIRS["step9ir_v2"]
REPLAY_LEDGER_DB = OUTPUT_DIR / "step9ir_v2_historical_replay_ledger.db"
RUN_LOG_FILE = OUTPUT_DIR / "step9ir_v2_replay_run_log.csv"
SUMMARY_FILE = OUTPUT_DIR / "step9ir_v2_replay_summary.csv"
DECISION_BATCH_FILE = OUTPUT_DIR / "step9ir_v2_replay_decision_batches.csv"
DECISION_FILE = OUTPUT_DIR / "step9ir_v2_replay_decisions.csv"
OUTCOME_BATCH_FILE = OUTPUT_DIR / "step9ir_v2_replay_outcome_batches.csv"
OUTCOME_FILE = OUTPUT_DIR / "step9ir_v2_replay_outcomes.csv"
DAILY_REGIME_FILE = OUTPUT_DIR / "step9ir_v2_replay_daily_regimes.csv"
DAILY_SUMMARY_FILE = OUTPUT_DIR / "step9ir_v2_replay_daily_summary.csv"
REGIME_STRATEGY_MATRIX_FILE = OUTPUT_DIR / "step9ir_v2_replay_regime_strategy_matrix.csv"
CONTRACT_PERFORMANCE_FILE = OUTPUT_DIR / "step9ir_v2_replay_contract_performance.csv"
TICKER_PERFORMANCE_FILE = OUTPUT_DIR / "step9ir_v2_replay_ticker_performance.csv"
SECTOR_PERFORMANCE_FILE = OUTPUT_DIR / "step9ir_v2_replay_sector_performance.csv"
CUMULATIVE_PNL_FILE = OUTPUT_DIR / "step9ir_v2_replay_cumulative_pnl.csv"
COMPARISON_FILE = OUTPUT_DIR / "step9ir_v2_replay_comparisons.csv"
AUDIT_FILE = OUTPUT_DIR / "step9ir_v2_replay_audit.csv"
CONTRACT_REGISTRY_FILE = OUTPUT_DIR / "step9ir_v2_replay_contract_registry.csv"
SEGMENT_PERFORMANCE_FILE = OUTPUT_DIR / "step9ir_v2_replay_segment_performance.csv"
SELF_INFLUENCE_FILE = OUTPUT_DIR / "step9ir_v2_replay_core5_regime_sensitivity.csv"
...
def build_audit(
    ...
    if outcomes.empty:
        eligible_core_rows = 0
        missing_core_candidates = 0
    else:
        core_mask = outcomes["ticker"].astype(str).isin(step9i.CORE_TICKERS)
        eligible_mask = outcomes["morning_contract_eligible"].map(_bool)
        eligible_core = outcomes[core_mask & eligible_mask]
        eligible_core_rows = int(len(eligible_core))
        missing_core_candidates = int((~eligible_core["candidate_generated"].map(_bool)).sum())
    extra = pd.DataFrame(
        [
            {
                "experiment_id": EXPERIMENT_ID,
            ...
            },
            {
                ...
                "interpretation": "Each sealed replay day contains all eight contracts for all 23 tradable tickers.",
            },
            {
                "experiment_id": EXPERIMENT_ID,
                "audit_item": "CORE5_ELIGIBLE_ROWS_REACH_EOD_CANDIDATE_ENGINE",
                "rows_checked": eligible_core_rows,
                "failures": missing_core_candidates,
                "audit_pass": missing_core_candidates == 0,
                "interpretation": (
                    "Every morning-eligible Core 5 ticker-contract row must reach the same EOD candidate engine "
                    "as the Holdout 18; otherwise Combined 23 performance is incomplete."
                ),
            },
            {
                "experiment_id": EXPERIMENT_ID,
...
def _read_core_sensitivity(ledger_db: Path) -> pd.DataFrame:
    if not Path(ledger_db).exists():
        return pd.DataFrame()
    with closing(sqlite3.connect(ledger_db)) as con:
        step9i._ensure_ledger_schema(con)
        return pd.read_sql_query("SELECT * FROM core_regime_sensitivity ORDER BY session_date, company_id", con)

def build_segment_performance(outcomes: pd.DataFrame, registry: pd.DataFrame) -> pd.DataFrame:
    frames = []
    for segment, tickers in (("CORE_5", step9i.CORE_TICKERS), ("HOLDOUT_18", step9i.HOLDOUT_ONLY_TICKERS), ("COMBINED_23", step9i.TRADING_TICKERS)):
        subset = outcomes[outcomes["ticker"].isin(tickers)].copy() if not outcomes.empty else outcomes.copy()
        performance = build_contract_performance(subset, registry)
        performance.insert(1, "universe_segment", segment)
...
def export_replay_outputs(
    ...
    if not ticker_performance.empty:
        ticker_performance.insert(ticker_performance.columns.get_loc("ticker") + 1, "universe_segment", ticker_performance["ticker"].map(step9i._segment_for_ticker))
    segment_performance = build_segment_performance(outcomes, registry)
    ...
    sensitivity = _read_core_sensitivity(ledger_db)
    expected_sensitivity = len(batches) * len(step9i.CORE_COMPANY_REMOVALS)
    sensitivity_audit = pd.DataFrame([{
        "experiment_id": EXPERIMENT_ID,
        "audit_item": "CORE5_REGIME_SELF_INFLUENCE_ROWS_RECORDED",
        "rows_checked": expected_sensitivity,
        "failures": max(0, expected_sensitivity - len(sensitivity)),
        "audit_pass": len(sensitivity) == expected_sensitivity,
        "interpretation": "Five leave-one-company-out regime sensitivity rows are recorded for each replay morning."
    }])
    audit = pd.concat([audit, sensitivity_audit], ignore_index=True)
    ...
    outputs = {
        ...
        str(SEGMENT_PERFORMANCE_FILE): segment_performance,
        str(SELF_INFLUENCE_FILE): sensitivity,
    }
...
def main() -> None:
    ...
    print("\n=== STEP 9I-R V2 HISTORICAL WALK-FORWARD REPLAY (23 TRADABLE TICKERS) ===")
    ...
    print("The replay is permanently non-confirmatory and cannot activate the router or alter the live Step 9I V2 ledger.")
    ...
    print("Use step9ir_v2_replay_regime_strategy_matrix.csv, step9ir_v2_replay_contract_performance.csv, and step9ir_v2_replay_segment_performance.csv to inspect the regime-strategy relationship.")
