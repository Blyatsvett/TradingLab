...
DAILY_FILE = DATA_DIR / "step9l_vs_step9i_v2_daily_comparison.csv"
SUMMARY_FILE = DATA_DIR / "step9l_vs_step9i_v2_comparison_summary.csv"
AUDIT_FILE = DATA_DIR / "step9l_vs_step9i_v2_comparison_audit.csv"
...
def _engine_daily(
    ...
    for batch in batches.to_dict("records"):
        ...
        primary_pnl = pd.to_numeric(primary.get("risk_capped_net_pnl_sek"), errors="coerce").fillna(0.0)
        guard_pnl = pd.to_numeric(guards.get("risk_capped_net_pnl_sek"), errors="coerce").fillna(0.0)
        rows.append(
            {
                "engine": engine_name,
                "session_date": date,
...
def build_comparison(
    ...
    research_ledger: Path = step9l.SHADOW_LEDGER_DB,
    ...
    research = _engine_daily("STEP9L_RESEARCH", r_batches, r_decisions, r_outcomes)
    ...
    if frozen.empty and research.empty:
        ...
        frozen_regime = daily["frozen_primary_regime"] if "frozen_primary_regime" in daily else pd.Series("", index=daily.index)
        research_regime = daily["research_primary_regime"] if "research_primary_regime" in daily else pd.Series("", index=daily.index)
        frozen_status = daily["frozen_prospective_status"] if "frozen_prospective_status" in daily else pd.Series("", index=daily.index)
        research_status = daily["research_prospective_status"] if "research_prospective_status" in daily else pd.Series("", index=daily.index)
    ...
    prospective = daily[daily.get("comparison_evidence", pd.Series(dtype=str)).eq("BOTH_ENGINES_PROSPECTIVE")].copy() if not daily.empty else daily
    summary = pd.DataFrame([
        {
            "comparison": "STEP9L_VS_STEP9I_V2_PRIMARY_HYPOTHESES",
    ...
    audit_rows = [
        {
            "audit_item": "FROZEN_AND_RESEARCH_LEDGERS_ARE_DISTINCT",
            "failures": 0 if Path(frozen_ledger).resolve() != Path(research_ledger).resolve() else 1,
            "audit_pass": Path(frozen_ledger).resolve() != Path(research_ledger).resolve(),
            "interpretation": "Step 9L never writes to the frozen Step 9I V2 ledger.",
        },
        {
    ...
    return daily, summary, pd.DataFrame(audit_rows)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare frozen Step 9I V2 with Step 9L.")
    parser.add_argument("--frozen-ledger", type=Path, default=step9i.SHADOW_LEDGER_DB)
    parser.add_argument("--research-ledger", type=Path, default=step9l.SHADOW_LEDGER_DB)
