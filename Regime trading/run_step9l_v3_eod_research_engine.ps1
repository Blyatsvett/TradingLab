...
$Arguments = @("-m", "RegimeTrading.scripts.step9l_v3_selected_strategy_shadow_engine", "eod")
if ($Date) { $Arguments += @("--date", $Date) }
if ($AsOf) { $Arguments += @("--as-of", $AsOf) }
if ($AllowEarlyEvaluation) { $Arguments += "--allow-early-evaluation" }
& $Python @Arguments
if ($LASTEXITCODE -ne 0) { throw "Step 9L V3 EOD research engine failed with exit code $LASTEXITCODE." }
