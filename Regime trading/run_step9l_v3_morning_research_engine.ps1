...
$Arguments = @("-m", "RegimeTrading.scripts.step9l_v3_selected_strategy_shadow_engine", "morning")
if ($Date) { $Arguments += @("--date", $Date) }
if ($AsOf) { $Arguments += @("--as-of", $AsOf) }
if ($AllowLateReconstruction) { $Arguments += "--allow-late-reconstruction" }
& $Python @Arguments
if ($LASTEXITCODE -ne 0) { throw "Step 9L V3 morning research engine failed with exit code $LASTEXITCODE." }
