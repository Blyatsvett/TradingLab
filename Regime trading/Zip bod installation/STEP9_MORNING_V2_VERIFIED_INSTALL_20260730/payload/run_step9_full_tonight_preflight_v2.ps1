...
function Compare-DatabaseState {
    ...
    if (($BeforeKeys -join "`n") -ne ($AfterKeys -join "`n")) {
        throw "The real project database or sidecar inventory changed during preflight."
    }
    foreach ($Key in $BeforeKeys) {
        if (($Before[$Key].sha256 -ne $After[$Key].sha256) -or
            ([long]$Before[$Key].length -ne [long]$After[$Key].length)) {
            throw "A real project database or sidecar changed during preflight: $Key"
        }
    }
}
...
try {
    ...
    $RuntimeClosureRequired = @(
        ...
        "tools/step9_morning_v2_support.py",
        "RegimeTrading/scripts/step9_morning_v2_stage_runner.py",
        "data/step9s_historical_contingency_replay_v1/freeze_v1/9b045fb10e196a38/step9s_historical_output_freeze_summary.json",
        "data/step9u_historical_contingency_selector_v1/freeze_8042ad803be28ccf/STEP9U_HISTORICAL_CONTINGENCY_SELECTOR_V1_FREEZE_MANIFEST.json",
        "config/step9s_prospective_contingency_shadow_v1.json",
        "config/step9r_candidate_ranking_research_v1.json",
        "config/step9t_prospective_regime_transition_archetype_v1.json",
        "config/step9u_prospective_contingency_selector_v1.json"
    ...
    Invoke-NativeChecked `
        ...
        -Arguments @(
            "-c",
            "from RegimeTrading.scripts import step9_morning_v2_stage_runner, step9i_v2_core5_plus_holdout18_shadow_router, step9l_v3_selected_strategy_shadow_engine, step9s_prospective_contingency_shadow_v1, step9r_v1_candidate_ranking_research, step9t_prospective_regime_transition_archetype_v1, step9u_prospective_contingency_selector_v1; print('STEP9_MORNING_V2_IMPORTS: PASSED')"
        ) `
        -WorkingDirectory $Root `
        -PythonPathRoot $Root `
