...
DEFAULT_PATHS = {
    "prices": DATA / "step9i_shadow_intraday_prices.db",
    "step9i": DATA / "step9i_v2_shadow_ledger.db",
    "step9l": DATA / "step9l_v3_selected_strategy_shadow_ledger.db",
    "step9s": DATA / "step9s_prospective_contingency_shadow_v1.db",
    "step9r": DATA / "step9r_prospective_selector_shadow_v1.db",
    "step9t": DATA / "step9t_regime_transition_archetype_prospective_v1.db",
    "step9u": DATA / "step9u_contingency_selector_prospective_shadow_v1.db",
}
...
def sqlite_backup(source: Path, destination: Path) -> dict[str, Any]:
    ...
    try:
        with closing(_ro_connect(source)) as src:
            ...
            if data_version_before != data_version_after:
                raise MorningV2Error(
                    "SQLite source changed while its isolated backup was being created."
                )
        source_hash_after = _sha256(source)
        if source_hash_before != source_hash_after:
            raise MorningV2Error(
                "SQLite source file changed while its isolated backup was being created."
            )
        os.replace(temp, destination)
    finally:
        temp.unlink(missing_ok=True)
