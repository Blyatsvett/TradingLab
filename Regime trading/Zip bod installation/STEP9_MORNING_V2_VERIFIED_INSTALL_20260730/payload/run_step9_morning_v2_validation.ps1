param(
    ...
    [string]$ReferenceMockRoot = "C:\Users\User\S9M\MOCK_20260730_PARTIAL_REAL_UPSTREAM_CONTINUATION_20260730_101448",
...
function Compare-DatabaseState {
    ...
    if (($BeforeKeys -join "`n") -ne ($AfterKeys -join "`n")) {
        throw "$Label database or sidecar inventory changed during validation."
    }
    foreach ($Key in $BeforeKeys) {
        if (($Before[$Key].sha256 -ne $After[$Key].sha256) -or
            ([long]$Before[$Key].length -ne [long]$After[$Key].length)) {
            throw "$Label database or sidecar changed during validation: $Key"
        }
    }
}
...
try {
    ...
    foreach ($Name in $ReferenceNames) {
        $Source = Join-Path $ReferenceMockRoot "data\$Name"
    ...
    foreach ($Spec in @(
        [pscustomobject]@{
            Source = Join-Path $ProjectRoot "data\step9i_v2_shadow_ledger.db"
            Destination = $RealIReference
            Label = "STEP 9I"
        },
        [pscustomobject]@{
            Source = Join-Path $ProjectRoot "data\step9l_v3_selected_strategy_shadow_ledger.db"
...
'@
    ...
    if ($StartupBenchmarkSeconds -gt $MaximumStartupBenchmarkSeconds) {
        throw (
            ...
            "second prewarm budget." -f
            $StartupBenchmarkSeconds,
            $MaximumStartupBenchmarkSeconds
        )
    }
    ...
    for ($Iteration = 1; $Iteration -le $Iterations; $Iteration++) {
        ...
        $IJob = Start-ValidationStage -Stage "step9i" -Iteration $Iteration -Arguments @(
            "--date", $Date,
            "--as-of", $AsOfI,
        ...
        $LJob = Start-ValidationStage -Stage "step9l" -Iteration $Iteration -Arguments @(
            "--date", $Date,
            "--as-of", $AsOfI,
            "--allow-late-reconstruction",
            "--source-db", $Snapshot0940Rel,
        ...
        $LResult = Complete-NativeProcess -Job $LJob -TimeoutSeconds $ChildTimeoutSeconds
        ...
        $IResult = Complete-NativeProcess -Job $IJob -TimeoutSeconds $ChildTimeoutSeconds
        $CriticalComplete = @(
        ...
        $Step9ICompleteOffset = (
            $DecisionStartOffset + [double]$IResult.duration_seconds
        )
        ...
        $ModeledCriticalSeconds = [math]::Max(
            $Step9ICompleteOffset,
            [math]::Max($Step9SCompleteOffset, $Step9UCompleteOffset)
        )
        ...
        if ([double]$MinimumModeledMargin -lt $MinimumLatestStartMarginSeconds) {
            throw (
                ...
                "required {1} seconds. S={2} T={3} U={4}" -f
        ...
        $IterationResults += [pscustomobject]@{
            ...
            timing_scope = "measured_stages_plus_live_schedule_model"
            snapshot_seconds = $SnapshotSeconds
            step9s_completion_from_iteration_start_seconds = $Step9SCompletionSeconds
            deadline_critical_i_l_s_t_u_seconds = $CriticalSeconds
            ...
            all_six_stages_seconds = $AllStageSeconds
    ...
    $WorstCritical = ($IterationResults |
        Measure-Object -Property deadline_critical_i_l_s_t_u_seconds -Maximum).Maximum
    ...
    $Report = [ordered]@{
        ...
        deadline_critical_stages = @("step9i", "step9l", "step9s", "step9t", "step9u")
        noncritical_stages = @("step9r")
