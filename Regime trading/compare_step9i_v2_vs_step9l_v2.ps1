...
$Arguments = @("-m", "RegimeTrading.scripts.step9l_v2_compare_step9i_v2")
if ($FrozenLedger) { $Arguments += @("--frozen-ledger", $FrozenLedger) }
if ($ResearchLedger) { $Arguments += @("--research-ledger", $ResearchLedger) }
& $Python @Arguments
if ($LASTEXITCODE -ne 0) { throw "Step 9I V2 versus Step 9L V2 comparison failed with exit code $LASTEXITCODE." }
