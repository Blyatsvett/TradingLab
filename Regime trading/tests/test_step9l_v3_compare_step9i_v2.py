...
from RegimeTrading.scripts import step9l_v3_compare_step9i_v2 as compare
...
class Step9LV3ComparisonTests(unittest.TestCase):
    def _ledger(
        self,
        path: Path,
        pnl: float,
        ...
        with closing(sqlite3.connect(path)) as con:
            ...
            con.execute(
                ...
                ("B", "2026-07-27", "PROSPECTIVE_CONFIRMATORY_ELIGIBLE", regime, 0.7),
            )
            con.execute(
    ...
    def test_comparison_uses_primary_pnl_and_detects_disagreement(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            research = Path(tmp) / "research_v3.db"
            self._ledger(frozen, -2.0, "FROZEN")
            self._ledger(
                research,
                3.0,
                "L3_TU_ALIGNED_DELAYED_REVERSAL_1R_V1",
            )
    ...
    def test_guardrails_are_not_mixed_into_engine_pnl(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertAlmostEqual(
                float(daily.iloc[0]["research_guardrail_counterfactual_pnl_sek"]),
                -9.0,
            )
