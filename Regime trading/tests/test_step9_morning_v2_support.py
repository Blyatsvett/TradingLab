...
def test_sqlite_backup_is_distinct_integral_and_read_only(
    ...
    assert result["source_opened_directly"] is False
    assert result["source_file_inventory"]["main"]["sha256"] == before
    assert mod._sha256(source) == before
    with sqlite3.connect(destination) as connection:
...
def test_sqlite_backup_does_not_open_or_mutate_wal_source(tmp_path: Path) -> None:
    source = tmp_path / "wal_source.db"
    destination = tmp_path / "wal_backup.db"
    connection = sqlite3.connect(source)
    try:
        assert connection.execute("PRAGMA journal_mode=WAL").fetchone()[0].lower() == "wal"
        connection.execute("PRAGMA wal_autocheckpoint=0")
        connection.execute("CREATE TABLE evidence(value TEXT)")
        connection.execute("INSERT INTO evidence VALUES ('wal-sealed')")
        connection.commit()
        before = mod._sqlite_source_state(source)
        assert "main" in before
        result = mod.sqlite_backup(source, destination)
        after = mod._sqlite_source_state(source)
        assert after == before
        assert result["source_opened_directly"] is False
        with sqlite3.connect(destination) as copied:
            assert copied.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
            assert copied.execute("SELECT value FROM evidence").fetchone()[0] == "wal-sealed"
    finally:
        connection.close()
