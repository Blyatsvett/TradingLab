...
def run_test(min_gap):
    ...
    daily_open["prev_open"] = daily_open.groupby("ticker")["day_open"].shift(1)
    daily_open["gap"] = daily_open["day_open"] / daily_open["prev_open"] - 1
    ...
    df = df.merge(daily_open[["ticker", "date", "gap"]], on=["ticker", "date"], how="left")
    ...
    print("\n" + "=" * 60)
    print(f"ORB GAP FILTER TEST: min gap {min_gap:.2%}")
    print("=" * 60)
...
for min_gap in [-0.01, -0.005, 0.0, 0.0025, 0.005, 0.01]:
    run_test(min_gap)