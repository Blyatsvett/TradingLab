...
def run_test(require_above_vwap=True):
    ...
    df["cum_tp_volume"] = df.groupby(["ticker", "date"])["tp_volume"].cumsum()
    df["cum_volume"] = df.groupby(["ticker", "date"])["volume"].cumsum()
    ...
    for (ticker, date), day in df.groupby(["ticker", "date"]):
        ...
        if require_above_vwap:
            breakout = breakout[breakout["close"] > breakout["vwap"]]
    ...
    label = "WITH VWAP FILTER" if require_above_vwap else "NO VWAP FILTER"

    print("\n" + "=" * 60)
    print(f"ORB VWAP FILTER TEST: {label}")
    print("=" * 60)
...
run_test(require_above_vwap=False)
run_test(require_above_vwap=True)