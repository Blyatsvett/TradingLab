...
def run_test(max_range_pct):
    ...
    orb["opening_range_pct"] = (
        orb["opening_high"] / orb["opening_low"] - 1
    )
    ...
    for (ticker, date), day in df.groupby(["ticker", "date"]):
        ...
        if pd.isna(opening_range_pct):
            continue

        if opening_range_pct > max_range_pct:
            continue
    ...
    print("\n" + "=" * 60)
    print(f"ORB RANGE FILTER TEST: max range {max_range_pct:.2%}")
    print("=" * 60)
...
for max_range in [0.005, 0.01, 0.015, 0.02, 0.03]:
    run_test(max_range)