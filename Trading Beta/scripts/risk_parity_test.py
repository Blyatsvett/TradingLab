...
def compute_inverse_vol_weights(selected):
    selected = selected.copy()

    selected["inv_vol"] = 1 / (selected["volatility"].abs() + 1e-9)
...
def simulate_risk_parity(
    ...
    for i in range(len(dates)):
        ...
        if should_rebalance:
            ...
            selected = compute_inverse_vol_weights(selected)
...
print("\n" + "=" * 60)
print("RISK PARITY TEST")
print("=" * 60)

equity_curve, daily_returns = simulate_risk_parity(
    df,
    top_n=3,
    rebalance_every=3,
    cost_per_rebalance=0.0005,
